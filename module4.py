    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            if not line:  # Skip blank lines
                continue
            name, amount_str = line.split(':')  # Split line into name and amount
            name = name.strip()  # Remove whitespace from name
            amount_str = amount_str.strip()  # Remove whitespace from amount
            try:
                amount = float(amount_str)  # Convert amount to float
            except ValueError:
                # Handle case where amount is not a valid number
                continue
            # Update bank dictionary
            if name in bank:
                bank[name] += amount
            else:
                bank[name] = amount
...
def signup(user_accounts, log_in, username, password):
    ...
    # Update user_accounts dictionary
    user_accounts[username] = password

    # Update log_in dictionary
...
def import_and_create_accounts(filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            if not line:  # Skip blank lines
                continue
            parts = line.split('-', 1)  # Split line into username and password
            if len(parts) != 2:  # Skip lines without exactly one '-'
                continue
            username = parts[0].strip()  # Remove whitespace from username
            password = parts[1].strip()  # Remove whitespace from password

            # Check if password is missing
            if not password:
                continue

            # Use the signup function to check and add the username and password
            if signup(user_accounts, log_in, username, password):
                log_in[username] = False  # Set initial login status
...
def login(user_accounts, log_in, username, password):
    ...
    log_in[username] = True  # Update log-in status
    return True
...
def update(bank, log_in, username, amount):
    ...
    if username not in bank:
        bank[username] = 0  # Create user if not exists

    # Check if the transaction won't result in a negative balance
...
def transfer(bank, log_in, userA, userB, amount):
    if userA not in log_in or not log_in[userA]:
        return False  # userA is not logged in or does not exist

    if userB not in log_in:
        return False  # userB does not exist

    if userA not in bank:
        return False  # userA is not in the bank

    if bank[userA] < amount:
        return False  # userA does not have enough balance
...
def delete_account(user_accounts, log_in, bank, username, password):
    ...
    # Delete user from dictionaries
    del user_accounts[username]
    del log_in[username]
    bank.pop(username, None)
